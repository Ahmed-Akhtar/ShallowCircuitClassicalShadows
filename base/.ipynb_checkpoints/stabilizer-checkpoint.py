...
def random_brickwall_layer(N,shift):
    '''construct random brickwall Clifford map of N qubits.
        Each two qubit clifford rotation is uniformly random.'''

    gates=[random_clifford_map(2) for copy in range(N//2)]
    gs=scipy.linalg.block_diag(*[gate.gs for gate in gates])
    ps=numpy.concatenate([gate.ps for gate in gates])
    if shift :
        gs=numpy.concatenate([gs[:,2:], gs[:,:2]],axis=1)
        gs=numpy.concatenate([gs[2:,:], gs[:2,:]],axis=0)
